import MyOnHandleFileOpen from "./customs/icons/MyOnHandleFileOpen";
import MyOnHandleUploadFile from "./customs/icons/MyOnHandleUploadFile";
import MyOnHandleFileExport from "./customs/icons/MyOnHandleFileExport";

import MyTooltipButton from "./MyTooltipButton";

function MyFileToolBar({ className }) {
  return (
    <div
      className={`shadow-md/5 w-full box-border bg-menu-bg border-b-1 border-menu-border text-[#767c8a] text-xs flex items-center justify-between fixed z-99 h-[32px] transition-all duration-300 ${className}`}
    >
      <div className="flex h-full">
        <MyTooltipButton tooltip="Open File">
          <MyOnHandleFileOpen />
        </MyTooltipButton>
        <div className="border-l-1 border-menu-border"></div>
        <MyTooltipButton tooltip="Import File">
          <MyOnHandleUploadFile />
        </MyTooltipButton>
        <MyTooltipButton tooltip="Export File">
          <MyOnHandleFileExport />
        </MyTooltipButton>
      </div>
    </div>
  );
}

export default MyFileToolBar;
